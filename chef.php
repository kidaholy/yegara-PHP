    ...
    <div class="max-w-screen-2xl w-full space-y-8">
        ...
        <div class="kitchen-ui glass p-8 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-8 bg-gray-900/40">
            <div class="flex items-center gap-6">
                ...
                <div>
                    <h1 class="text-3xl lg:text-4xl font-bold text-white leading-tight mt-1">Food Kitchen</h1>
                    <p class="text-sm font-medium text-gray-400 mt-2">Kitchen Queue &amp; Order Fulfillment</p>
                </div>
            </div>
        ...
        <div class="kitchen-ui grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="glass p-6 rounded-2xl bg-gray-800/80 hover:bg-gray-800 transition-colors border border-gray-700/50">
                <div class="flex items-center justify-between mb-4">
                    <p class="text-sm font-medium text-gray-400">Chef On Duty</p>
                    <div class="inline-flex p-3 rounded-lg bg-gray-900 border border-gray-700 text-blue-400">
        ...
        <div class="kitchen-ui glass p-6 rounded-2xl border border-gray-700/50 bg-gray-800/60">
            <p class="text-xs font-bold uppercase tracking-widest text-gray-500 mb-4">Chef Kitchen Categories</p>
        ...
        <div id="kitchen-queue-panel" class="glass p-8 rounded-2xl border border-blue-900/40 bg-blue-950/20">
            <div class="kitchen-ui flex items-center justify-between mb-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-blue-400 mt-1">Kitchen Queue <span id="queue-count-label" class="text-blue-500"></span></h3>
                        <p class="text-sm font-medium text-blue-400/60 mt-1">Today's pending food orders</p>
                    </div>
                </div>
            </div>
...
<script>
        ...
        if (!queue.length) {
            row.innerHTML = `
                <div class="col-span-full flex flex-col items-center justify-center w-full py-16 text-center">
                    ...
                    <p class="text-lg font-bold text-gray-300">Kitchen is clear</p>
                    <p class="text-sm text-gray-500 mt-1">No food orders waiting right now</p>
                </div>`;
            lucide.createIcons();
            return;
        }
        ...
        row.innerHTML = queue.map(order => `
            <article class="glass flex flex-col rounded-xl border border-blue-900/30 bg-gray-900/60 hover:bg-gray-900 transition-colors overflow-hidden h-full">
                ...
                <div class="p-4 border-t border-gray-700/50 shrink-0">
                    <button type="button" data-serve-id="${esc(order.id)}" data-serve-num="${esc(order.orderNumber)}"
                            class="serve-btn w-full py-3 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2">
                        <i data-lucide="circle-check" class="w-4 h-4"></i> Serve
                    </button>
                </div>
            </article>
        `).join('');
    ...
    async function serveOrder(id, num) {
        try {
            ...
            const json = await resp.json();
            if (!resp.ok) throw new Error(json.message || 'Failed to update order');
            refreshQueue();
        } catch (err) {
            alert(err.message || 'Could not serve order');
        }
    }
    ...
    document.getElementById('queue-row').addEventListener('click', e => {
        ...
        if (!btn) return;
        serveOrder(btn.dataset.serveId, btn.dataset.serveNum);
    });
